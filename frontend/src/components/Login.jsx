...
import {userQueryEmail} from '../utils/data';
const Login = () => {
  ...
  const responseGoogle = (response) => {
    ...
    const decoded = jwt_decode(response.credential); // updated JWT decoding
    ...
    //localStorage.setItem('user', JSON.stringify(decoded)); // store who logged in
    ...
    client.fetch(userQueryEmail(email)) //Query the backend for users having the given email
      .then(users => {
  ...
  //NOTES:
  //useEffect HOOK: asynchronous runs a function every render of the component -- is a way to run code on every render
  ...
  useEffect(() => {
    //NOTES:
    //global google says that variable google is a "global variable" -- even though it's not declared anywhere in the program -- so it doesnt cause issues
